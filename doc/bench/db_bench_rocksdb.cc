...
class Benchmark {
 private:
  ...
  void PrintWarnings() {
    ...
    if (!port::Snappy_Compress(Options().compression_opts, text, sizeof(text), &compressed)) {
 ...
 private:
  ...
  void SnappyCompress(ThreadState* thread) {
    ...
    while (ok && bytes < 1024 * 1048576) {  // Compress 1G
      ok = port::Snappy_Compress(Options().compression_opts, input.data(), input.size(), &compressed);
