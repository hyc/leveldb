...
class Benchmark {
 ...
 private:
  ...
  void PrintEnvironment() {
    fprintf(stdout, "MDBM:       version 4.11.1\n");
  ...
  : db_(NULL),
    ...
    if (!FLAGS_use_existing_db) {
      for (int i = 0; i < files.size(); i++) {
        if (Slice(files[i]).starts_with("dbbench_mdbm")) {
  ...
  void Run() {
    ...
    if (fresh_db) {
	  ...
	  if (db_) {
		  ...
		  mdbm_close(db_);
		  mdbm_delete_lockfiles(FLAGS_db);
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < n; i++) {
	  ...
	  arg[i].thread->dbt = mdbm_dup_handle(db_, 0);
  ...
  void Write(ThreadState *thread) {
	...
	datum mkey, mval;
	char key[100];
	int flag = MDBM_REPLACE;

	int64_t bytes = 0;
    // Write to database
	unsigned long i = 0;
	while (!duration.Done(entries_per_batch_)) {
	  MDBM_ITER it = MDBM_ITER_INITIALIZER;

	  mdbm_lock(thread->dbt);
	  for (int j=0; j < entries_per_batch_; j++) {

      const unsigned long k = (write_order_ == SEQUENTIAL) ? i+j : (shuff ? shuff[i+j] : (thread->rand.Next() % FLAGS_num));
	  int rc;
	  mkey.dptr = key;
	  mkey.dsize = snprintf(key, sizeof(key), "%016lx", k);
      bytes += value_size_ + mkey.dsize;

	  mval.dptr = (char *)gen.Generate(value_size_).data();
	  mval.dsize = value_size_;
	  rc = mdbm_store_r(thread->dbt, &mkey, &mval, flag, &it);
      if (rc) {
	  ...
	  mdbm_unlock(thread->dbt);
	  if (dbflags_ == SYNC)
	  	mdbm_fsync(thread->dbt);
  ...
  void ReadSequential(ThreadState *thread) {
	...
	kvp = mdbm_first_r(thread->dbt, &it);
	do {
	  *(volatile char *)kvp.val.dptr;
      bytes += kvp.key.dsize + kvp.val.dsize;
      thread->stats.FinishedSingleOp();
	  kvp = mdbm_next_r(thread->dbt, &it);
    } while (kvp.key.dsize && kvp.val.dsize);
	thread->stats.AddBytes(bytes);
  }
  ...
  void BGWriter(ThreadState* thread) {
	...
	while (true) {
	  ...
	  mval.dsize = value_size_;
	  mdbm_lock(thread->dbt);
	  rc = mdbm_store_r(thread->dbt, &mkey, &mval, MDBM_REPLACE, &it);
	  mdbm_unlock(thread->dbt);
	  if (rc) {
