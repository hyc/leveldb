...
class Benchmark {
 ...
 private:
  ...
  void PrintEnvironment() {
    fprintf(stderr, "ForestDB:   version 1.0b\n");
...
//	fconfig.compaction_threshold = 0;
  ...
  void Write(ThreadState *thread) {
	...
	while (!duration.Done(entries_per_batch_)) {
	  rc = fdb_begin_transaction(db_, FDB_ISOLATION_READ_COMMITTED);	// we want serializable!
	  if (rc) {
	  	fprintf(stderr, "begin_txn error: %d\n", rc);
		exit(1);
	  }
	  ...
	  rc = fdb_end_transaction(db_, FDB_COMMIT_NORMAL);
      if (rc) {
        fprintf(stderr, "commit error: %d\n", rc);
		break;
      }
  ...
  void ReadSequential(ThreadState *thread) {
    ...
    while ((rc = fdb_iterator_next(iter, &doc)) == 0) {
      bytes += doc->keylen + doc->bodylen;
	  fdb_doc_free(doc);
      thread->stats.FinishedSingleOp();
    }
  ...
  void BGWriter(ThreadState* thread) {
	...
	while (true) {
	  ...
	  rc = fdb_commit(db_, FDB_COMMIT_NORMAL);
	  if (rc) {
		fprintf(stderr, "commit error: %d\n", rc);
		exit(1);
	  }
