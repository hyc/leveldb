...
class Benchmark {
  ...
  : db_(NULL),
    ...
    if (!FLAGS_use_existing_db) {
      for (int i = 0; i < files.size(); i++) {
        if (Slice(files[i]).starts_with("dbbench_sph")) {
 ...
 private:
	...
	db_ = sp_env();
	rc = sp_ctl(db_, SPDIR, SPO_RDWR|SPO_CREAT, file_name);
	if (rc) {
      fprintf(stderr, "sp_ctl error: %s\n", sp_error(db_));
    }
	dbi_ = sp_open(db_);
	if (!dbi_) {
  ...
  void Write(DBFlags flags, Order order, DBState state,
    ...
    for (int i = 0; i < num_entries; i+= entries_per_batch)
    {
      ...
      bytes_ += value_size + ksz;

	  value = gen_.Generate(value_size).data();
	  rc = sp_set(dbi_, key, ksz, value, value_size);
      if (rc) {
  ...
  void ReadReverse() {
	...
	cursor = sp_cursor(dbi_, SPLTE, "9999999999999999", 16);
    while (sp_fetch(cursor) == 1) {
	  const char *key, *value;
	  int ksz, vsz;
	  key = sp_key(cursor);
	  ksz = sp_keysize(cursor);
	  value = sp_value(cursor);
	  vsz = sp_valuesize(cursor);
      bytes_ += ksz + vsz;
      FinishedSingleOp();
    }
	sp_destroy(cursor);
  ...
  void ReadSequential() {
	...
	cursor = sp_cursor(dbi_, SPGTE, NULL, 0);
    while (sp_fetch(cursor) == 1) {
	  const char *key, *value;
	  int ksz, vsz;
	  key = sp_key(cursor);
